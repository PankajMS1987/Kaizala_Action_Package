...
var _strings;
var _form;
var genderValueList = ['Male', 'Female'];
var attachmentList = [];
function onPageLoad() {
...
function inflateQuestions() {
    ...
    inflateInputBox(document.getElementById("nameDiv"), _form.questions[0].title, 'Tap to enter', function(event) {
        _nameAns = event.target.value;
    });
    
    inflateDropDown(document.getElementById("genderDiv"), _form.questions[1].title, "Tap to select", genderValueList, function(event) {
        _gender = event.target.value;
    });
    
    inflateAttachmentDiv(AttachmentDiv, _form.questions[2].title, attachmentList, 'picAttachment');

    inflateInputBox(document.getElementById("otherQuestionDiv"), _form.questions[3].title, 'Tap to enter', function(event) {
        _otherAns = event.target.value;
    });
}
...
function submitFeedback() {
    ...
    ans['0'] = _nameAns;
    ans['1'] = _gender;
    ans['2'] = JSON.stringify(attachmentList);
    ans['3'] = _otherAns;
    KASClient.Form.sumbitFormResponse(ans, null, false, false, false);
}
